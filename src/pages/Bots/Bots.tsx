          ...
          <div key={bot.id} className="bot-card">
            <div className="bot-card-header">
              ...
              <div className="bot-info">
                <h3 className="bot-name">{bot.name ?? bot.id}</h3>
                <p className="bot-project">{bot.project ?? '—'}</p>
              </div>
              <span className={`bot-status status-${bot.status ?? 'unknown'}`}>
                {bot.status === 'online' ? '在线' : bot.status === 'training' ? '训练中' : '未知'}
              </span>
            </div>
              ...
              <div className="bot-score">
                <div className="bot-score-bar">
                  <div
                    ...
                    style={{ width: `${bot.score ?? 0}%` }}
                  />
                </div>
                <span className="bot-score-text">训练度 {bot.score ?? 0}%</span>
              </div>
            </div>
