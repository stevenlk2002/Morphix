...
export default function KnowledgeTab({ bot }) {
  ...
  const handleSave = async (payload, editingId) => {
    if (editingId) {
      ...
      setKnowledge(
        knowledge.map((item) => (item.id === editingId ? updated : item))
      )
    } else {
...
function KnowledgeModal({ initial, onClose, onSave }) {
