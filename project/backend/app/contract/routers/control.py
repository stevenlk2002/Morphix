...
from app.contract.services.orchestration import iso, start_manual_run
...
def _to_detail(db: Session, conv: Conversation) -> ConversationDetail:
    ...
    if isinstance(conv.contact, dict):
        contact_dto = ContactRef(
            external_uid=conv.contact.get("externalUid") or "",
            display_name=conv.contact.get("displayName") or "",
